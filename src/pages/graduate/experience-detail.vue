  ...
  <view class="post-detail">
    <view class="post-card">
      <view class="post-header">
        <view class="avatar">
          ...
          <text v-else>{{ (postData.nickname || '匿')[0] }}</text>
        </view>
        <view class="user-info">
          <text class="nickname">{{ postData.nickname }}</text>
          <view class="tag-row">
            <text class="tag" v-if="postData.school">{{ postData.school }}</text>
            <text class="tag success" v-if="postData.admitted">已上岸</text>
            <text class="tag cat" v-if="postData.category">{{ postData.category }}</text>
          </view>
          <text class="time">{{ postData.time }}</text>
        </view>
      </view>
      <text class="post-title" v-if="postData.title">{{ postData.title }}</text>
      <text class="post-content">{{ postData.content }}</text>
      <view class="post-images" v-if="postData.images.length">
        <image v-for="(img, i) in postData.images" :key="i" class="img-item" :src="img.url" mode="aspectFill" @click="previewImg(i)" />
      </view>
    ...
    <view class="input-bar">
      ...
      <view class="input-row">
        <input v-model="newComment" :placeholder="inputPlaceholder" class="comment-input" :focus="inputFocus"></input>
...
const postData = ref({
  avatar: '', nickname: '加载中...', time: '', title: '', content: '',
  images: [], openid: '', school: '', admitted: false, category: ''
})
const comments = ref([])
...
const loadDetail = async (id) => {
  var res = await callCloud('experience', 'detail', { id: id })
  if (res.code === 0) {
    var d = res.data
    postData.value = {
      avatar: d.avatar || '',
      ...
      title: d.title || '',
      content: d.content || '',
      images: (d.images || []).filter(function(img) { return img && typeof img === 'string' && (img.indexOf('cloud://') === 0 || img.indexOf('https://') === 0 || img.indexOf('http://') === 0) }).map(function(img) { return { url: img } }),
      openid: d.openid || '',
      school: d.school || '',
      admitted: d.admitted || false,
      category: d.category || ''
    }
    likeCount.value = d.likes || 0
    liked.value = (d.likedBy || []).indexOf(uni.getStorageSync('openid') || '') !== -1
    comments.value = (d.commentList || []).map(function(c) {
      var myOpenid = uni.getStorageSync('openid') || ''
      return {
        _id: c._id || '', openid: c.openid || '',
        avatar: c.avatar || '', name: c.nickname || '匿名',
        time: formatTime(c.createTime), content: c.content || '',
        likes: c.likes || 0, liked: (c.likedBy || []).indexOf(myOpenid) !== -1,
        replyTo: c.replyTo || '', replyName: c.replyName || ''
      }
    })
  }
...
var formatTime = (t) => {
  ...
  if (y === now.getFullYear()) return (m < 10 ? '0' + m : m) + '-' + (day < 10 ? '0' + day : day) + ' ' + (hh < 10 ? '0' + hh : hh) + ':' + (mm < 10 ? '0' + mm : mm)
...
const previewImg = (idx) => {
  uni.previewImage({ current: idx, urls: postData.value.images.map(i => i.url) })
}

const toggleLike = async () => {
  if (!checkLogin()) return
  var res = await callCloud('experience', 'like', { postId: postId.value })
  if (res.code === 0) { liked.value = res.liked; likeCount.value += res.liked ? 1 : -1 }
}

const toggleFavorite = async () => {
  if (!checkLogin()) return
  var res = await callCloud('user', 'toggleFavorite', { targetId: postId.value, targetType: 'post' })
  if (res.code === 0) { favorited.value = res.favorited; uni.showToast({ title: res.favorited ? '已收藏' : '取消收藏', icon: 'success' }) }
}
...
const sendComment = async () => {
  ...
  if (replyTarget.value._id) { params.replyTo = replyTarget.value._id; params.replyName = replyTarget.value.name }
  var res = await callCloud('experience', 'comment', params)
  if (res.code === 0) {
    comments.value.unshift({ _id: res.id || '', openid: uni.getStorageSync('openid') || '', avatar: '', name: '我', time: '刚刚', content: newComment.value, likes: 0, liked: false, replyTo: replyTarget.value._id || '', replyName: replyTarget.value.name || '' })
    newComment.value = ''
    cancelReply()
...
var startReply = (c) => { if (!checkLogin()) return; replyTarget.value = { _id: c._id, name: c.name }; inputPlaceholder.value = '回复 ' + c.name + '...'; inputFocus.value = true }
var cancelReply = () => { replyTarget.value = { _id: '', name: '' }; inputPlaceholder.value = '写评论...'; inputFocus.value = false }

var toggleCommentLike = async (c) => {
  if (!checkLogin()) return
  if (!c._id) return
  var res = await callCloud('experience', 'likeComment', { commentId: c._id })
  if (res.code === 0) { c.liked = res.liked; c.likes += res.liked ? 1 : -1 }
}
...
const deleteComment = (c, index) => {
  uni.showModal({ title: '删除评论', content: '确定要删除这条评论吗？', success: async (res) => {
    if (!res.confirm) return
    if (!c._id) { comments.value.splice(index, 1); return }
    var delRes = await callCloud('experience', 'deleteComment', { commentId: c._id })
    if (delRes.code === 0) { comments.value.splice(index, 1); uni.showToast({ title: '已删除', icon: 'success' }) }
  }})
}
</script>
...
.avatar { width: 80rpx; height: 80rpx; border-radius: 50%; background: linear-gradient(135deg, #68D391, #38A169); display: flex; align-items: center; justify-content: center; font-size: 36rpx; color: #fff; margin-right: 16rpx; overflow: hidden; }
.avatar-img { width: 80rpx; height: 80rpx; border-radius: 50%; }
.user-info { flex: 1; }
.nickname { font-size: 30rpx; font-weight: bold; color: #333; display: block; }
.tag-row { display: flex; gap: 8rpx; margin-top: 6rpx; flex-wrap: wrap; }
.tag { font-size: 20rpx; color: #718096; background: #F0F2F5; padding: 4rpx 12rpx; border-radius: 6rpx; }
.tag.success { color: #38A169; background: #F0FFF4; }
.tag.cat { color: #2B6CB0; background: #EBF8FF; }
.time { font-size: 22rpx; color: #999; margin-top: 4rpx; display: block; }
.post-title { font-size: 34rpx; font-weight: 700; color: #1A1A2E; display: block; margin-bottom: 16rpx; line-height: 1.4; }
.post-content { font-size: 30rpx; color: #333; line-height: 1.8; margin-bottom: 20rpx; white-space: pre-wrap; }
...
.comment-avatar { width: 64rpx; height: 64rpx; border-radius: 50%; background: #F0F2F5; display: flex; align-items: center; justify-content: center; font-size: 28rpx; color: #718096; margin-right: 16rpx; flex-shrink: 0; overflow: hidden; }
...
.reply-target { font-size: 22rpx; color: #38A169; font-weight: 600; }
...
.reply-hint-text { font-size: 22rpx; color: #38A169; font-weight: 500; }
...
.send-btn { margin-left: 16rpx; padding: 16rpx 32rpx; background: linear-gradient(135deg, #43A047, #2E7D32); border-radius: 36rpx; }
