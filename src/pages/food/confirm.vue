...
const deliveryFee = computed(() => cartData.value.deliveryFee || 0)
const totalPrice = computed(() => itemsTotal.value + deliveryFee.value)
...
const submitOrder = async () => {
  ...
  if (!address.value.trim()) { uni.showToast({ title: '请填写地址', icon: 'none' }); return }
  ...
  if (res.code === 0) {
    ...
    setTimeout(() => {
      uni.redirectTo({ url: '/pages/errand/detail?id=' + res.data.orderId })
    }, 1200)
  }
}
